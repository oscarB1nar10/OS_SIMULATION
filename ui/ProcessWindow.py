...
class ProcessWindow:
    ...
    def __init__(self, win):
        ...
        self.input_process_id.place(x=210, y=50)
        self.lb_process_name.place(x=100, y=100)
        self.input_process_name.place(x=210, y=100)
        self.lb_process_weight.place(x=100, y=150)
        self.input_process_weight.place(x=210, y=150)
        self.lb_process_threads.place(x=100, y=200)
        self.input_process_threads.place(x=210, y=200)
        self.lb_process_resources.place(x=100, y=250)
        self.resources_menu.place(x=210, y=245)
        ...
        self.btn_return_home = Button(win,text='Return',command=self.return_home)
        #self.btn_process_stats = Button(win, text='Process stats', command=self.stats)
        self.btn_create_process.place(x=40, y=310)
        self.btn_new_process.place(x=180, y=310)
        self.btn_execute_process.place(x=300, y=310)
    ...
    def execute(self):
        ...
        self.lb_process_id.destroy()
        self.lb_process_name.destroy()
        self.lb_process_id.destroy()
        self.lb_process_threads.destroy()
        self.lb_process_weight.destroy()
        self.lb_process_resources.destroy()
        self.input_process_id.destroy()
        self.input_process_weight.destroy()
        self.input_process_name.destroy()
        self.input_process_threads.destroy()
        self.btn_new_process.destroy()
        #self.btn_process_stats.destroy()
        self.btn_execute_process.destroy()
        self.btn_create_process.destroy()
        self.resources_menu.destroy()
        self.process_frame.destroy()
        self.btn_return_home.place(x=10, y=5)
        window.title('Start Process')
        window.iconbitmap('boton-de-play.ico')
    ...
    def return_home(self):
        print('No se que hacer o poner aqui')
...
my_canvas.create_window((40,30), window=third_frame, anchor="nw")

my_win = ProcessWindow(main_frame)
window.title('Create Process')
window.iconbitmap('home_icon.ico')
